...
import Index, { SectionPage } from "./pages/Index";
...
const pages = [
  ...
  ["contact", "Contact", "Du möchtest zusammenarbeiten oder einfach Hallo sagen? Schreib xJanne."],
] as const;

const App = () => <QueryClientProvider client={queryClient}><TooltipProvider><Toaster /><Sonner /><BrowserRouter><Routes><Route path="/" element={<Index />} />{pages.map(([path, title, subtitle]) => <Route key={path} path={`/${path}`} element={<SectionPage title={title} subtitle={subtitle} />} />)}<Route path="*" element={<NotFound />} /></Routes></BrowserRouter></TooltipProvider></QueryClientProvider>;
