...
import Index, { ContactPage, SectionPage } from "./pages/Index";
...
const App = () => <QueryClientProvider client={queryClient}><TooltipProvider><Toaster /><Sonner /><BrowserRouter><Routes><Route path="/" element={<Index />} /><Route path="/contact" element={<ContactPage />} />{pages.map(([path, title, subtitle]) => <Route key={path} path={`/${path}`} element={<SectionPage title={title} subtitle={subtitle} />} />)}<Route path="*" element={<NotFound />} /></Routes></BrowserRouter></TooltipProvider></QueryClientProvider>;
