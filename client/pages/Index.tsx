...
const sectionContent: Record<string, { image: string; label: string; links: { label: string; href: string; external?: boolean }[] }> = {
  Socials: { image: "https://cdn.builder.io/api/v1/image/assets%2F843c2c9147174e21a08386d68870f8b1%2Faec16121e65a4908a9979850982e9fb7?format=webp&width=800&height=1200", label: "FOLGE XJANNE", links: [{ label: "xJanneTV auf Twitch", href: "https://www.twitch.tv/xjannetv", external: true }, { label: "Discord Community", href: "/contact" }] },
  Projekte: { image: "https://cdn.builder.io/api/v1/image/assets%2F843c2c9147174e21a08386d68870f8b1%2F8421c898a7de468283b88454a2878a27?format=webp&width=800&height=1200", label: "AKTUELLE WELTEN", links: [{ label: "Arcanimals entdecken", href: "/arcanimals" }, { label: "Kunst ansehen", href: "/kunst" }] },
  Stream: { image: "https://cdn.builder.io/api/v1/image/assets%2F843c2c9147174e21a08386d68870f8b1%2Fdb67860ef05d445daf98bb9eebb8a36f?format=webp&width=800&height=1200", label: "LIVE AUF TWITCH", links: [{ label: "xJanneTV öffnen", href: "https://www.twitch.tv/xjannetv", external: true }] },
  Kunst: { image: "https://cdn.builder.io/api/v1/image/assets%2F843c2c9147174e21a08386d68870f8b1%2F173803265ac54437a1c3cbad6d006298?format=webp&width=800&height=1200", label: "VISUELLE WELTEN", links: [{ label: "Projekte ansehen", href: "/projekte" }] },
  Musik: { image: "https://cdn.builder.io/api/v1/image/assets%2F843c2c9147174e21a08386d68870f8b1%2Fd1bb8734c134498e9d5644135108cb61?format=webp&width=800&height=1200", label: "SOUNDS VON XJANNE", links: [{ label: "Im Stream dabei sein", href: "/stream" }] },
  Arcanimals: { image: "https://cdn.builder.io/api/v1/image/assets%2F843c2c9147174e21a08386d68870f8b1%2Fef6cba0444bd416596d3192e9ba7745e?format=webp&width=800&height=1200", label: "EINE MAGISCHE WELT", links: [{ label: "Mehr Projekte", href: "/projekte" }] },
};

export function SectionPage({ title, subtitle }: { title: string; subtitle: string }) {
  const content = sectionContent[title];
  return <div className="site-shell"><Header /><main className="section-page"><div className="section-page-inner"><span className="eyebrow">XJANNE / {title.toUpperCase()}</span><h1>{title}</h1><p>{subtitle}</p><div className="section-content"><img src={content.image} alt="" /><div className="section-links"><span className="contact-label">{content.label}</span>{content.links.map((link) => <a key={link.href} href={link.href} target={link.external ? "_blank" : undefined} rel={link.external ? "noreferrer" : undefined} className="text-link">{link.label} <ArrowUpRight size={17} /></a>)}</div></div><Link to="/" className="text-link">Zur Startseite <ArrowUpRight size={17} /></Link></div></main><Footer /></div>;
}
